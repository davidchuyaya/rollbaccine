...
def outfile(is_primary: bool) -> str:
    if is_primary:
        return f"results/crash_primary_out_postgres.txt"
    return f"results/crash_backup_out_postgres.txt"
...
def run(recover_primary: bool):
    ...
    unique_str = f"recoverPrimary{recover_primary}postgres"
    ...
    main_outfile = outfile(recover_primary)
    ...
    print(f"Messing up the disk with 100M of zeros")
    success = ssh_executor.exec(main_ssh, [
        "sudo umount /dev/sdb1",
        f"sudo dd if=/dev/zero of=/dev/sdb bs=1G count=100"
    ])
    if not success:
        return False
